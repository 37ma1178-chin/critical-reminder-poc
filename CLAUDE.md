...
## Files
- `data/urbania-17.json` — the single source of truth for every dimension (mm). `body` = confirmed spec, `assumptions` = estimates (cab depth, wall thickness and hood length were scaled off the reference render), `stock_seating` = seat counts and positions, matched to the Force Motors 4400WB 16+D render (`confirmed: true`, see its `confirmation` block for what that means: pattern exact, positions ±50 mm). Row model: aisle can be off-centre (`aisle_center_offset`), a row can lack its left seat (`rows_without_left_seat`, the sliding door), left singles can sit behind the right pairs (`left_row_offset`), `seat_depth` is the footprint including the backrest.
- `data/reference/` — the Force Motors top-view and side-profile renders the layout was matched to, plus the annotated measurement image.
