...
- `js/vehicle.js` — loads both JSON files and derives all shared geometry once (`Vehicle.load()` → `{ spec, layout, components, camper }`): axle positions, interior box, wheel rectangles, the full list of seat rectangles, and the camper zones (`camperZones`) laid out rear-wards from the partition. `priceRange` sums catalog prices. Both views draw from this, so they cannot disagree. Vehicle frame: x = 0 at the rear bumper increasing to the front, y = 0 on the centreline increasing to the right (driver) side.
- `index.html` — 3D viewer (Three.js r128 via jsdelivr UMD). Cutaway / roof-off (camera flies overhead, roof slab hidden) / full exterior views, stock 17-seat layout, lounge (still a placeholder preset), camper conversion built from the catalog zones. Converts mm → m; scene x = vehicle x − length/2, scene z = vehicle y.
...
4. Make camper modules individually toggleable instead of one bundled "camper conversion" preset (per-module price from `Vehicle.priceRange`; draw the zones in `topview.html` too).
